...
class IntegerRange:
    ...
    def __get__(self, instance: SlideLimitationValidator, owner: object) \
            -> SlideLimitationValidator:
        return getattr(instance, self.protected_name)

    def __set__(self, instance: SlideLimitationValidator, value: int) -> None:
        print("\n__set__: type(instance):", type(instance))
        if not (self.min_amount <= value <= self.max_amount):
            raise ValueError(f"{self.protected_name[1:]} is out "
                             f"of its allowed range of {self.min_amount} "
